...
import com.gxp.jt808.vo.req.TerminalRegisterMsg.TerminalRegInfo;
...
public class MsgDecoder {
	...
	public PackageData bytes2PackageData(byte[] data) {
		...
		if (checkSumInPkg != calculatedCheckSum) {
			log.warn("检验码不一致,msgid:{},flowId:{},pkg:{},calculated:{},hex:{}", msgHeader.getMsgId(),msgHeader.getFlowId(), checkSumInPkg, calculatedCheckSum,HexStringUtils.toHexString(data));
		}
		return ret;
	}
	...
	private String parseStringFromBytes(byte[] data, int startIndex, int lenth, String defaultVal) {
		try {
			...
			log.error("解析字符串出错:{}", e.getMessage(),e);
			return defaultVal;
		}
	}
	...
	public LocationInfoUploadMsg toLocationInfoUploadMsg(PackageData packageData) {
		...
		ret.setLatitude(this.parseIntFromBytes(data, 8, 4)*1.0d/100_0000);
		// 4. byte[12-15] 经度(DWORD(32)) 以度为单位的经度值乘以10^6，精确到百万分之一度
		//		ret.setLongitude(this.parseFloatFromBytes(data, 12, 4));
		ret.setLongitude(this.parseIntFromBytes(data, 12, 4)*1.0d/100_0000);
		...
		ret.setDateTime(DateUtil.convertDateToString(date,"yyyy-MM-dd HH:mm:ss"));
		return ret;
	}
